  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="space-y-4">
        {/* Dose & Water */}
        {((sf?.coffeeDose ?? true) && (sf?.totalWater ?? true)) && (
        ...
        {((sf?.bloomWater ?? true) || (sf?.bloomTime ?? true)) && (
...
export default function NewBrew() {
  ...
  useEffect(() => {
    ...
    const setupId = searchParams?.get("setup") ?? null;
    if (setupId) {
    ...
    if (isEditMode && editId) {
      ...
      createBrewLog.mutate(payload, {
        onSuccess: (result: { id?: string }) => {
          ...
          setShowSuccess(true);
          setTimeout(() => {
            router.push(successNavRef.current!);
          }, 1200);
        },
      });
    }
  }, [state, selectedBean, createBrewLog, updateBrewLog, router, isEditMode, editId, editBrew]);
